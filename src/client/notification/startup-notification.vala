...
/*
 * A simple class for manipulating startup notifications for new mails through a simple
 * desktop file at $HOME/.config/autostart/geary.desktop
 */
public class StartupNotification : Object{
    ...
    private string config_dir; // User-specific configuration directory, usually '$HOME/.config/'

    public StartupNotification() {
        config_dir = Environment.get_user_config_dir();
        startup_file = File.new_for_path(config_dir).get_child(AUTOSTART_FOLDER).get_child(AUTOSTART_DESKTOP_FILE);
    ...
    public void sync_with_config() {
        if (startup_file.query_exists()){
           GearyApplication.instance.config.startup_notifications = true; 
        }else {
           GearyApplication.instance.config.startup_notifications = false; 
        }
    }
}
